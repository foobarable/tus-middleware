...
exports.createServer = function(opts) {
    ...
    function createFile(req, res) {
        getPositiveIntHeader(req, "Upload-Length").then(function(uploadLength) {
            if (options.maxFileSize != null && uploadLength > options.maxFileSize) {
                throw new BadRequest("File exceeds maximum allowed file size of " + options.maxFileSize + " bytes");
            }
            return createEmptyFile(crypto.randomBytes(16).toString('hex'), options.directory, uploadLength, {contentType: req.get('Content-Type') || null, filename: req.get('Entity-Name') || null});
        }).then(function(name) {
